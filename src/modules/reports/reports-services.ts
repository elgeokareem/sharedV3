...
export const fetchSecuredCaseBySpotters = async (
  startDate: string,
  endDate: string,
): Promise<{
  camerasByBranch: any;
  ...
  securedByUserList = statusCalc(securedByUserList, previousSecuredByUserList);
  ...
  const camerasByBranch = groupCamerasByBranch(
    { all_hits: allHits, secured: securedByUserList, scanned: cameraScansList },
    idToBranch,
  );
